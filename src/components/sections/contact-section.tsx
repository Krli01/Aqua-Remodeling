...
const MapWidget: React.FC = () => (
  <div className="w-full h-full min-h-75 overflow-hidden rounded-3xl shadow-xl border border-(--color-primary)/10">
    <iframe
      title="Aqua Remodeling Location"
      src="https://www.google.com/maps?q=3831+SW+160th+Ave,+Miramar,+FL+33027&output=embed"
...
export const ContactSection: React.FC<{ isHomePage?: boolean }> = ({
  isHomePage = false,
}) => {
          ...
          <div className="grid lg:grid-cols-12 gap-8 lg:gap-12 items-stretch">
            <div className="lg:col-span-7">
              <form
                ...
                className="flex flex-col gap-5 bg-white rounded-3xl p-6 md:p-10 shadow-xl border border-(--color-primary)/10 relative overflow-hidden h-full"
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mt-4">
                  <div className="space-y-1.5">
                    <label
                      htmlFor="name"
                      className="block text-sm font-semibold text-[rgb(100,98,92)]"
                    >
                      {t("contact.form.name")}
                    </label>
                    <input
                      ...
                      required
                      className="w-full px-4 py-3.5 bg-(--color-bg-light) border-transparent rounded-xl text-sm text-(--color-bg-dark) outline-none transition-all focus:bg-white focus:ring-2 focus:ring-(--color-primary)/30 focus:border-(--color-primary) placeholder-[rgb(150,148,142)]"
                    />
                  </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    required
                    rows={4}
                    className="w-full px-4 py-3.5 bg-(--color-bg-light) border-transparent rounded-xl text-sm text-(--color-bg-dark) outline-none transition-all focus:bg-white focus:ring-2 focus:ring-(--color-primary)/30 focus:border-(--color-primary) placeholder-[rgb(150,148,142)] resize-none"
                  />
                </div>
            ...
            <div className="lg:col-span-5 flex flex-col gap-6">
              <div className="bg-(--color-bg-dark) text-white rounded-3xl p-8 md:p-8 shadow-xl relative overflow-hidden flex-1">
                ...
                <div className="space-y-5 relative z-10">
                    ...
                    <div>
                      ...
                      <div className="space-y-1 mt-0.5">
                        <a
                          ...
                          className="text-white hover:text-(--color-primary) font-medium inline-block transition-colors"
                        >
                          {CONTACT_INFO.PHONE_DISPLAY}
                        </a>
                        <p className="text-[11px] text-gray-400">
                          {t("contact.info.english_service")}
                        </p>
                        <a
                          href={CONTACT_INFO.PHONE_SPANISH_CALL}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-white hover:text-(--color-primary) font-medium inline-block transition-colors"
                        >
                          {CONTACT_INFO.PHONE_SPANISH_DISPLAY}
                        </a>
                        <p className="text-[11px] text-gray-400">
                          {t("contact.info.spanish_service")}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-white hover:text-(--color-primary) font-medium mt-0.5 inline-block break-all transition-colors"
                      >
                        {CONTACT_INFO.EMAIL}
                      </a>
                    </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-white mt-0.5">{CONTACT_INFO.HOURS}</p>
                    </div>
                  </div>
                </div>
              </div>

              <div className="h-56 rounded-3xl overflow-hidden shadow-lg border border-(--color-primary)/10 min-h-40 relative">
                <MapWidget />
              </div>
            </div>
          </div>
    ...
    >
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-8 relative z-10">
        <div className="text-center mb-12">
          ...
          <p className="text-[rgb(100,98,92)] mt-3 max-w-2xl mx-auto">
            {t("contact.subtitle") ||
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
          <div className="bg-white rounded-2xl p-5 shadow-lg border border-(--color-primary)/10 text-center hover:shadow-xl transition-shadow">
            ...
            <h3 className="font-bold text-(--color-bg-dark) text-sm">
              {t("contact.info.contact_title")}
            </h3>
            <div className="space-y-2">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="text-(--color-primary) text-sm font-medium hover:underline block"
              >
                {CONTACT_INFO.PHONE_DISPLAY}
              </a>
              <p className="text-xs text-[rgb(100,98,92)]">
                {t("contact.info.english_service")}
              </p>
              <a
                href={CONTACT_INFO.PHONE_SPANISH_CALL}
          ...
          <div className="bg-white rounded-2xl p-5 shadow-lg border border-(--color-primary)/10 text-center hover:shadow-xl transition-shadow">
            ...
            <p className="text-[rgb(100,98,92)] text-sm">
              {CONTACT_INFO.LOCATION}
            </p>
          </div>
      ...
      {(status === "success" || status === "error") && (
        <div
          className={`fixed bottom-8 right-8 z-100 flex items-center gap-3 px-6 py-4 rounded-2xl shadow-2xl border transition-all duration-500 animate-in fade-in slide-in-from-bottom-5 ${
            // eslint-disable-line tailwindcss/enforces-shorthand
            status === "success"
              ? "bg-white border-green-100 text-green-800"
              : "bg-white border-red-100 text-red-800"
          }`}
